...
public class VendorBillMasterQuery 
{
     public void insertIntoVendorBillMaster(VendorBillMaster vendorBillMaster)
     {
        ...
        try
        {
            ...
            session.saveOrUpdate(vendorBillMaster);
            transaction.commit();
        }
        catch(Exception e)
        {
    ...
    public List<Object[]> getTotalPurchase(VendorBillMaster vendorBillMaster, String taxType)
    {       
        ...
        query =  "SELECT date, billAmount, discount, finalBillAmount FROM VendorBillMaster \n" +
                     "WHERE ((date BETWEEN '" + sqlFromDate +"' AND '"+ sqlToDate + "') AND remark = '"+taxType+"') \n" +
                     " ORDER BY date ASC";
//        else
...
//                     "GROUP BY date ORDER BY date ASC";
    ...
    {
        String query = "FROM StockDetailsPurchase sd JOIN sd.vendorBillMaster vbm JOIN sd.purchaseMaster im JOIN vbm.vendorMaster "
